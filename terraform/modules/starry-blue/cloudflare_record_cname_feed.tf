...
resource "mackerel_service" "feed" {
  name = "Lily_TinyTinyRSS"
}

resource "mackerel_monitor" "feed" {
  name = format("%s に疎通できない", cloudflare_record.cname_feed.hostname)

  external {
    method                 = "GET"
    url                    = format("https://%s", cloudflare_record.cname_feed.hostname)
    service                = mackerel_service.feed.name
